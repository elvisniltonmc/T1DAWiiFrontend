    ...
    public String autenticando(
        ...
        try {
            ...
            if (tipoDocumento == null || tipoDocumento.trim().length() == 0 ||
                ...
                LoginModel loginModel = new LoginModel("01", "Error: Debe completar bien sus credenciales", "Cesar Santos");
                model.addAttribute("loginmodel", loginModel);
                return "iniciando";  // Asegúrate de que la vista 'iniciando' existe
            }

            // Si la validación es exitosa
            LoginModel loginModel = new LoginModel("00", "", "Cesar Santos");
            model.addAttribute("loginmodel", loginModel);
            return "home";  // Asegúrate de que la vista 'home' existe
        } catch (Exception e) {
            // Manejo del error
            model.addAttribute("errorMessage", "Ocurrió un error inesperado: " + e.getMessage());
            return "error";  // Retorna a una página de error personalizada
        }
    }
